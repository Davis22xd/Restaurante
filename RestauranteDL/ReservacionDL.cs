...
namespace RestauranteDL
{
    public class ReservacionDL
    {
        ...
        public DataSet BuscarTodo()
        {
            try
            {
                sentencia = "Select a.IDRESERVACION, a.IDCLIENTE, b.NOMBRECLIENTE, a.FECHAHORARESERVACION, a.ESTADORESERVACION from RESERVACION a inner join CLIENTE b on a.IDCLIENTE = b.IDCLIENTE";
                conexion.Open();
                comando = new SqlCommand(sentencia, conexion);
        ...
        {
            int idGenerado;
            sentencia = "select max (IDRESERVACION) from RESERVACION";
            conexion.Open();
            comando = new SqlCommand(sentencia, conexion);
            comando.CommandType = CommandType.Text;
